...
// Drone Geometries
const DRONE_BODY_GEO = new THREE.BoxGeometry(0.2, 0.1, 0.2);
const DRONE_LENS_GEO = new THREE.CylinderGeometry(0.08, 0.08, 0.1, 16);
const DRONE_PROP_GEO = new THREE.BoxGeometry(0.02, 0.01, 0.4);

const CameraDrone: React.FC = () => {
    const droneRef = useRef<THREE.Group>(null);
    const propRefs = useRef<THREE.Mesh[]>([]);
    
    useFrame((state, delta) => {
        if (droneRef.current) {
            // Hover animation independent of player bobbing
            droneRef.current.position.y = 1.8 + Math.sin(state.clock.elapsedTime * 2) * 0.1;
            // Smooth follow delay effect (simple oscillation for "stabilizer" look)
            droneRef.current.rotation.z = Math.sin(state.clock.elapsedTime * 1) * 0.05;
    ...
    return (
        <group ref={droneRef} position={[0.6, 1.8, 0.5]}>
            {/* Body */}
            <mesh geometry={DRONE_BODY_GEO}>
                 <meshStandardMaterial color="#222" metalness={0.8} roughness={0.2} />
            </mesh>
            {/* Lens */}
            <mesh rotation={[Math.PI/2, 0, 0]} position={[0, 0, -0.1]} geometry={DRONE_LENS_GEO}>
            ...
            <pointLight position={[0.08, 0.05, -0.1]} color="#FF6D00" intensity={1} distance={1} decay={2} />

            {/* Props */}
            {[-1, 1].map(x => [-1, 1].map(z => (
                <group key={`${x}-${z}`} position={[x * 0.15, 0.05, z * 0.15]}>
                    <mesh 
                        ref={el => { if(el) propRefs.current.push(el) }}
                        geometry={DRONE_PROP_GEO}
                    >
                        <meshBasicMaterial color="#444" transparent opacity={0.8} />
                    </mesh>
                </group>
            )))}
        </group>
...
export const Player: React.FC = () => {
  ...
  // Memoized Materials - UPDATED to Warm "Pioneer" Colors (Orange/Purple)
  const { armorMaterial, jointMaterial, glowMaterial, shadowMaterial } = useMemo(() => {
      const armorColor = isImmortalityActive ? '#ffd700' : '#FF6D00'; // Orange armor
      const glowColor = isImmortalityActive ? '#ffffff' : '#D500F9'; // Purple Glow
      
      return {
          armorMaterial: new THREE.MeshStandardMaterial({ color: armorColor, roughness: 0.3, metalness: 0.6 }),
          jointMaterial: new THREE.MeshStandardMaterial({ color: '#1A1A1A', roughness: 0.7, metalness: 0.5 }),
